...
| `.claude/commands/gov/analyze.md` | Generated | Regenerated by the pre-commit hook — never hand-edited |
...
**The tell list is fixed with no per-project extension.** Acknowledged during clarification as the same shape as Family 6's hardcoded SSOT list, which 026's own plan concedes is "a §design-principles violation in miniature". Accepted because the check is advisory: a false positive costs a glance, not a blocked gate. The revisit trigger is a project reporting domain prose that trips a tell repeatedly.
