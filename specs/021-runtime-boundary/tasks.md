...
## 8. Run `/ductus:analyze` against this spec

- [x] Run `/ductus:analyze` targeted at `021-runtime-boundary` and resolve any hard-fail or blocking findings.
- [x] Anchor-resolution check passes for `§runtime-boundary` references.
- **Done when**: `/ductus:analyze` reports no hard-fail and no blocking findings on this spec.
