...
- [x] AC5: An anchor marker `<!-- §runtime-boundary -->` exists at the new subsection so `/gov:analyze`'s anchor-resolution check resolves references to it.
...
- [x] AC10: `/gov:analyze` against this spec passes (no hard-fail or blocking findings) once acceptance criteria are met.
