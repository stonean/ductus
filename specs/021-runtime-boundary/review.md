...
`lint-*.sh` reuses the convention established by `gen-*.sh` (017): single-purpose bash script, `--help` flag, structured exit codes. The frontmatter integrity check is intentionally shape-only — the rigorous parser is `/ductus:analyze`'s hard-fail tier; this lint is a CI smoke test, not a duplicate.
