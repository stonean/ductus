...
- [x] State explicitly that Step 1 (rule item), Step 2 (new spec → `/ductus:specify`), and the Step 4 chore branch set **no** target. (New `### Setting the session target` subsection; Scope Boundaries updated to note the `.govern.session.toml` write.)
