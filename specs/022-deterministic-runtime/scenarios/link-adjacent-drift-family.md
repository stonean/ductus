...
**Evaluation is per link, and a finding requires an actual contradiction.** A block carrying three links is scanned three times and fires only for the target whose readable state contradicts the tell. Question-state tells are contradicted by an open-question count of zero; implementation-state tells by a spec target at `in-progress` or `done`; the existence tell by a target file that is present. Every other pairing yields nothing.
...
- The existence tell fires on the *link target's* existence, not on the subject of the sentence — prose saying a worker `does not exist` while linking to a scenario that does is flagged on the artifact. The message therefore states what was actually checked rather than implying the subject was verified.
