...
- `/ductus` (§Instructions step 6) MUST write `project` to the committed `.govern.toml` `[host]` block (dropping any legacy `cli-config-dir` the managed block previously carried) and write `cli-config-dir` to `.govern.session.toml` via a host-config write. On the markdown-only path the host performs both writes directly.
...
- **Mixed-agent team.** Contributor A (Claude) and contributor B (OpenCode) share one repo. Each runs `/ductus`; each writes their own `.govern.session.toml` `cli-config-dir` (`.claude` vs `.opencode`). The committed `.govern.toml` `[host]` carries only `project`, identical for both. Neither clobbers the other.
