...
- `read-tasks` MUST recognize a task's "Done when" clause in every form the writers and the corpus produce, and record its body as `done_when`:
  ...
  - `- [x] Done when: <body>` / `- [ ] Done when: <body>` — the checkbox-nested form `/ductus:plan`'s task breakdown tends to author.
...
- Prevention (framework side, outside the runtime): the `tasks.md` template carries a `- **Done when**: …` example line and `/ductus:plan`'s task-breakdown reference names the exact marker, so the primary writer converges on the canonical form the reader prefers.
