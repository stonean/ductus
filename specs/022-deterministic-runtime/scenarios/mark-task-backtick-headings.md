...
- **Backtick at start of title** (``### 7. `ductus` self-test``): the dot-separator (`.`) is the parser's discriminator, not the leading character of the title — backtick at position 0 of the title is benign once heading parsing uses the shared helper.
