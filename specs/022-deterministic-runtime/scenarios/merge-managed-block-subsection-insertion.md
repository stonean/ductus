...
The merge converges (a second run reaches `unchanged` — no infinite accumulation), so the damage is a one-time injection of permanent orphan comment headers, not unbounded growth. New adopters (no marker on disk) are unaffected: the `inserted` path writes the new canonical verbatim. Only existing adopters re-running `/ductus` hit it.
