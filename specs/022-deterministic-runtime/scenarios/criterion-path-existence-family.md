...
- **Residual, measured rather than assumed:** an adopter-layout path whose top-level segment *also* exists in the framework repo (`.githooks/govern-pre-commit`, `.govern/constitution.md`) still flags, because the `root-absent` rule cannot see that the criterion is about a different checkout. Sixteen such findings survived the first full-repo run. The class exists only where a repo documents adopter layout while also being its own adopter, and it is the reason the promotion criterion's precision half currently reads *do not promote*.
