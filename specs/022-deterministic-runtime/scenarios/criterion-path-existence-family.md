...
- **Residual, measured rather than assumed:** an adopter-layout path whose top-level segment *also* exists in the framework repo (`.ductus/constitution.md`, `specs/rules/security-backend.md`, `.githooks/ductus-pre-commit`, `specs/system.md`) still flags, because `root-absent` cannot see that the criterion describes a different checkout. Nineteen of the 28 findings in the 2026-08-03 full-repo run are this class — and every one of those paths is something `ductus` *creates in an adopter project*, absent here only because `ductus` is the source rather than an adopter. The class is structural to a framework repo documenting adopter layout while also being its own adopter, and it does not generalize to the projects this check ships to. It is why the promotion verdict reads *do not promote, and re-measure in an adopter repo*: this repo is the least representative sample the check will ever run against.
