...
4. `/ductus`'s bootstrap (`framework/bootstrap/ductus.md`) writes the `[host]` block into the adopter's `.govern.toml` during the install. The block is idempotent — re-runs update existing values rather than appending duplicate sections. Existing `.govern.toml` files in adopter projects (created before this scenario lands) gain the block on their next `/ductus` run.
5. Fixtures under `runtime/tests/fixtures/` that exercise `ductus exec` and anchor resolution gain a `.govern.toml` with explicit `[host]` values so the parity tests cover both Claude (`.claude`/`gov`) and Auggie (`.augment`/`anvil`) shapes. The Auggie fixture is the regression test for this scenario.
...
**Source-of-truth shape — `.govern.toml` `[host]` block (Option 1).** Resolved 2026-05-24 during task 41 implementation. The runtime loads `Host { cli_config_dir, project }` from a `[host]` block in `.govern.toml`, defaulting to `.claude` / the repo directory basename when the block is missing. `/ductus`'s bootstrap writes the block idempotently. Option 2 (env vars) was rejected as invisible at the repo level; Option 3 (per-invocation CLI flags) was rejected for ergonomic cost.
