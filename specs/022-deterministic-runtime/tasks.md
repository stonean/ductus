...
- [ ] `check_artifacts.rs`: extend the `review-state-drift` family with a staleness arm on `done` specs — a resolvable `reviewed-against` with durable contracts changed since it, reported **advisory** (the existing unset/blocking arms stay blocking). **Re-argue the case first**: with the exemption in place both existing mechanisms report 0 on this corpus, so the arm's motivating premise no longer holds as written.
- [ ] Resolve staleness through the same path `check-review-gate`'s `ReviewStale` uses — `stale_review_block` + `is_durable_contract`, **not** `compute_review_scope::read_plan_affected` (that scoping was tried and rejected at 34-of-48; see the scenario's first Resolved Question) — one implementation, never a second matching rule
