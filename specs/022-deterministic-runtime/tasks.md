...
- [ ] Version bump + `runtime/CHANGELOG.md` + `gvrn-v*` tag. Nothing here reaches an adopter until the tag ships; run `scripts/audit/run-all.sh` locally first, since the tag pipeline treats it as a hard release gate.
