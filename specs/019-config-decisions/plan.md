...
The `/ductus` runbook tells the agent how to read, parse, edit, and write `.govern.toml`. There is no `ductus`-side parser to update, no helper library, no test harness. The change is to the instructions the agent follows.

**Implication for the plan:** every "behavior change" in the spec maps to a prose edit in `framework/bootstrap/ductus.md`. The plan tasks are edit-shaped, not code-shaped. Verification is reading the prose against the acceptance criteria, plus a bench-test pass where one of us walks the runbook against a synthetic `.govern.toml` to confirm the steps are unambiguous.
...
- **No commit hook or `/ductus:analyze` rule for `.govern.toml`.** The summary-line surface is the only enforcement layer, by Q9.
...
- **No structured TOML validator.** We rely on the agent's literal adherence to runbook prose. If an adopter hand-edits `.govern.toml` into something the runbook's loose parsing can't handle (unusual whitespace, multi-line arrays with comments), the worst case is the load step fails to find the entries and `/ductus` falls back to "no declines recorded" — the prompts re-fire. This is annoying but not destructive. The TOML parse-error abort path remains the hard fail mode.
