...
- [x] Confirm the preceding release is complete before starting: a pushed `ductus-v0.37.0` tag whose workflow published assets. If it is not, finish or unwind it first — do not bump past it
- [x] Add a fresh `## [Unreleased]` section to `runtime/CHANGELOG.md` for this spec's primitives (the annotation writer, the `retire-feature` gate, the reciprocity family). The `merge-permissions` `revoke` entry is **not** pending — it shipped under `[0.37.0]`
