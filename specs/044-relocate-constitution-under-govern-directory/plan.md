# 044 — Relocate the shipped constitution to `.ductus/constitution.md` Plan

Implements [044 — Relocate the shipped constitution to `.ductus/constitution.md`](spec.md).

## Overview

A markdown-and-registry change with zero runtime code changes: the Shared Files manifest row moves the shipped constitution's destination from the adopter repo root to `.ductus/constitution.md`, every shipped artifact that names the adopter-root path is rewritten (seed templates, command bodies, bootstrap prose, framework docs), and a new `constitution-relocate` migration converges existing adopters. The gvrn 0.24.0 cut exists to anchor the migration's `introduced_in` (the 043 precedent for a no-behavior-change version); the parity suite is expected to pass untouched.

## Technical Decisions

### Manifest row and bootstrap prose

The ductus-owned Shared Files table maps `framework/constitution.md` → `constitution.md` (`framework/bootstrap/ductus.md:614`); the row's destination becomes `.ductus/constitution.md`. The `[pinned] files` schema example that illustrates pinning with `"constitution.md"` (`ductus.md:432`) is updated to the new destination path so the example stays copy-pasteable. No other `ductus.md` reference names the adopter destination — the remaining constitution mentions cite the framework source (`framework/constitution.md`, e.g. `ductus.md:520`), which is unchanged.
...
Shipped command bodies read or cite the constitution by the bare adopter-root path; each becomes `.ductus/constitution.md`: `target.md:19`, `specify.md:25,68`, `groom.md:64`, `clarify.md:100`, and `analyze.md:123,178,216,220,228` plus `analyze.md:48`'s explicit dual-path rule (adopter side only — `framework/constitution.md` in ductus's own repo stays). The sweep is grep-driven at implement time (`rg -n '\bconstitution\.md'` over `framework/`, excluding `framework/constitution.md` self-references and relative links that resolve to the framework source, e.g. `framework/migrations/spec-and-plan-sunset.md:26`'s `../constitution.md`) so no hidden reference survives — my scoping grep filtered out lines that mention both forms, so the implement-time sweep must not.

### Seed templates

The four seed-template references listed above (claude-md.md:3, agents.md:9,53, project-readme.md:20,35) are updated to `.ductus/constitution.md`. Per the resolved discoverability question, no new governance blurb is added — the README seed's lead Documentation bullet already carries the role description.

### Ductus's own docs — including a pre-existing dead link

- `AGENTS.md:9` links `[constitution.md](constitution.md)` — a **dead link today**: ductus's repo carries no root `constitution.md` (verified by `ls`; its `CLAUDE.md` imports `framework/constitution.md` directly). Fix it to `framework/constitution.md` as part of this sweep.
- `AGENTS.md:15`'s "sync target of root `constitution.md`" wording is stale for the same reason; reword to name `.ductus/constitution.md` as the adopter-side destination of the shipped source.
- `README.md:210,217,284` use `constitution.md` as the pinned-file and `update`-strategy examples; update to `.ductus/constitution.md`.
...
| File | Action | Purpose |
| --- | --- | --- |
| `framework/bootstrap/ductus.md` | Modify | Shared Files row → `.ductus/constitution.md`; pinned schema example |
...
| `framework/templates/project/claude-md.md` | Modify | `@import .ductus/constitution.md` |
...
- **No read fallback (vs 042's indefinite runtime fallback).** Rejected adding one: no runtime primitive reads the constitution, and command bodies are rescaffolded in the same `/ductus` run that moves the file, so readers and location cut over atomically. The residual risk — a pinned command body frozen on the old path — is handled by a named warning, not silent breakage.
...
- **`.markdownlint-cli2.jsonc` stays at root.** Moving it under `.ductus/` would demand `--config` plumbing in every lint invocation across all host paths for a purely aesthetic gain — out of scope, carved out in the spec.
