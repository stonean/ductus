...
Spec 030's harvest generator `scripts/gen-cross-service-refs.sh` (tasks.md §3) harvests body-link references into the derived `references:` index by matching each link's repo against a registered `.govern.toml [services]` entry and extracting the linked spec from the URL path. The extraction is a hardcoded `/specs/NNN-slug/` matcher — it assumes the *referenced* service keeps its ductus artifacts under `specs/`.
