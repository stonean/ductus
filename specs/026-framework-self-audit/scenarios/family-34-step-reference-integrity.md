...
A new `/{project}:audit` family asserts that every `step N` reference in a command file resolves to a numbered step that file actually has.
...
- **Step set:** derived from the file's own top-level `^\d+\.` plus a space Instructions headings, never hardcoded. A hardcoded expectation would be a second copy of the fact under test.
...
- **The markdown-only reference sections renumber independently.** Several command files carry a `## Markdown-only reference` whose sub-procedures restart at 1. Those are separate numbered lists, not Instructions steps, so the step set is scoped to the Instructions section and references inside the markdown-only section are resolved against their own list.
