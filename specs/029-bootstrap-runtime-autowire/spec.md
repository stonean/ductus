...
- [x] AC18: Project inputs (`name`, `description`, `languages`) are persisted in `.govern.toml`'s `[project]` table and resolved at §Collect Project Inputs after the Pre-flight Phase — reading existing values back and prompting only for what is missing — so they are asked at most once across the State B / stale-`ductus.md` restart and never re-asked on update runs. (`host.project` is the derived runtime namespace, written from `project.name`.) *(scenario: [project-inputs-asked-once](scenarios/project-inputs-asked-once.md))*
