...
- **Every step that names a backticked primitive** (a bare name — `fetch-archive`, `extract-archive`, `apply-manifest`, `merge-managed-block`, `enforce-manifest`, `substitute-templates`, `merge-permissions`, `merge-claude-md`, `run-generator`, … — matching a `ductus` tool in the inventory) **MUST be performed by calling that MCP tool**.
...
- **Per-step graceful degradation**: if a primitive call errors (e.g. a too-old wired `ductus` raises a parse error per spec 022 §Versioning enforcement), fall back to that one step's shell spec and continue — the whole run does not abandon the deterministic path.
