...
The top-level directory that holds all ductus artifacts is hardcoded to `specs`. This feature adds a single operator-set `.govern.toml` setting for that directory's name, chosen at initial configuration, defaulting to `specs` so existing projects are unaffected. Every command and the optional runtime resolve the spec root from the setting instead of a hardcoded literal.
...
- **Chosen at initial configuration.** During [003-bootstrap-automation](../003-bootstrap-automation/spec.md) (`/ductus`), the operator may set the spec-root name; the prompt defaults to `specs` and persists the answer to `.govern.toml`. The prompt lives only in the configuration command — no other command asks for it.
...
- [x] AC2: At initial configuration (`/ductus`), the operator can choose a different spec-root name; the prompt defaults to `specs` and the choice is persisted to `.govern.toml`. No command other than `/ductus` prompts for it.
...
- [033-rule-surface-setting](../033-rule-surface-setting/spec.md) — analogous `.govern.toml` operator setting with a `/ductus` prompt and a derive-by-default posture; a close structural model for this feature.
