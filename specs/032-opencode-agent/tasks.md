...
- [x] Add the `opencode` row to `framework/bootstrap/ductus.md` §Agent Registry (`config_dir: .opencode`, `layout: opencode`, `settings_template`, `rules_file_note`) per [data-model.md](data-model.md)
- [x] Add the `opencode` column to §Derived values (command path `.opencode/command/{project}/<name>.md`, invocation `/{project}/<name>`, `ductus` install path `.opencode/command/ductus.md`, settings file root `opencode.json`, permission shape OpenCode action map, native rules dir —, native rules file `AGENTS.md`, cleanup glob `{project}/` subdir under `command/`)
...
- [x] Branch §MCP wiring's `write-file` subsection with an `opencode` sub-case: write the `mcp` key with OpenCode's server shape (`{ "type": "local", "command": ["ductus", "mcp"], "enabled": true }`) into root `opencode.json`, additively (missing-file / has-key-no-ductus / already-present no-op / no-key / invalid-JSON-skip)
- [x] Add `"ductus*": "allow"` to the §ductus-runtime-auto-wiring permission grant list for OpenCode
- Done when: ductus.md documents OpenCode's project-committed write-file MCP wiring (root `opencode.json`, OpenCode shape) and the auto-wire grant, additively and host-side
...
- [x] Scaffold the `ductus` installer to `.opencode/command/ductus.md` (verbatim, placeholders kept literal)
- [x] Branch Self-Update Check + Post-Write Integrity Check claude-style-like (install path `.opencode/command/ductus.md`, direct byte compare, `# ductus` first-line check — no frontmatter strip)
...
- [x] Author the configure command writing root `opencode.json` `permission` (action map: `edit: allow`, `bash` allow/deny patterns, `webfetch`/`websearch` as needed, `"ductus*": "allow"` ordered after any broad `*`)
...
- [x] Add `opencode.md` as a fourth splice target; emit a constant single `"ductus*": "allow"` line (built outside the per-tool loop, like Antigravity's `mcp(ductus/*)`) between the markers
...
- [x] Update the illustrative gitignore-block references in `framework/bootstrap/ductus.md` (steps that list `.claude/`, `.augment/`, `.agents/`)
...
- [x] Add the `opencode)` case: `dest=".opencode/command/ductus.md"`, `mkdir -p .opencode/command`, seed root `opencode.json` `permission` (only if absent)
- [x] Update the usage comment and the unknown-agent error message to include `opencode`
- Done when: `curl … | sh -s -- opencode` installs a verbatim `/ductus` command and a seeded `opencode.json`

## 9. Document the OpenCode bootstrap in README

- [x] Add an OpenCode curl snippet (`… | sh -s -- opencode`) and an `### OpenCode` install section (installed as `.opencode/command/ductus.md`; reads AGENTS.md)
...
- [x] Extend `scripts/tests/test-gen-configure-mcp.sh` to assert the `opencode.md` `"ductus*"` block is emitted and in sync
...
- [x] Walk the spec acceptance criteria; confirm each is satisfied (including a live `opencode mcp list` → `ductus` connected check on a scaffolded sample)
...
<!-- Out of scope (tracked, not a task here): extending ductus `exec`'s `Host`
     ...
     runtime change. OpenCode ships on the markdown-only path; ductus MCP tools are
