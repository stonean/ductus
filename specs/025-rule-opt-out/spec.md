...
1. **Pin the file in `.govern.toml [pinned] files` and delete its content locally.** Defeats the purpose of shipping rules — the next `ductus` update can't push improvements to a file the adopter is now maintaining.
...
- [x] AC6: `/ductus:status` surfaces the disabled list (when present) in the pipeline dashboard, so the override is visible at-a-glance and doesn't hide in `.govern.toml`.
- [x] AC7: `/ductus:analyze` does NOT error on the new key. The key is a `.govern.toml` extension owned by this spec, not a spec-frontmatter change.
