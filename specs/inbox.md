...
- [ ] other: nothing detects a later spec invalidating an earlier spec's acceptance criterion. `check-artifacts`' `criterion-path-existence` family fires only when a criterion names a path that is *also* live-assertion-shaped and has a resolvable root segment — 010's AC7/AC8 (the `[simple]` marker convention, dropped by `017-derive-dont-ask`) escaped all three filters by accident, and were only annotated because a human re-derived them at the completion gate. Traceability is missing in the other direction too: 017 declares no dependency on 010, so nothing links remover to removed, and the sole record of the supersession sat in `review.md`'s Summary until a re-run of `/ductus:review` regenerated it away. Same shape as 027's `migration-chain-reference-integrity` one level up — specs rather than migrations. Surfaced 2026-08-16 closing 010.
