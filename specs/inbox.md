...
- [ ] doc drift: framework/commands/analyze.md §"Unexamined targets" lists the skipped-reason set without `ships-to-adopter`, which the runtime emits and 022's data-model documents — the adopter-facing reference is missing a reason its own runtime produces (found while adding the criterion-labels family, 2026-08-14)
- [ ] constitution gap: §spec-lifecycle defines a mechanical (no back-edge) edit as exactly two diff-determinable cases — uniform token substitution and cross-service-reference edits. 013 declares the `AC{n}:` label sweep mechanical and it swept 47 done specs without reopening them, so the rule and the practice now disagree; a reader of the constitution alone would conclude those specs drifted (found during the 013 backfill, 2026-08-14)
