...
- [ ] convention: `/gov:amend`'s reconcile pass matches scenario→task linkage host-side (read `tasks.md`, match `scenarios/{slug}.md` in task text), duplicating the derivation `check-artifacts`' `scenario-consistency` family owns in the runtime — no shared primitive, so the two can drift if either side's matching changes (e.g. a task body that references the scenario by a different form). A `scenario-tasks`-style primitive returning per-scenario referencing tasks with checkbox state would give both one source, and would be runtime work routed to 022 per AGENTS.md. — framework/commands/amend.md, runtime/src/primitives (captured during 023/extend-existing-scenario-task)
