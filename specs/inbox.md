...
- [ ] bug: a spec numbered past 999 is created with a name its own membership predicate rejects — `create-feature` formats `{number:03}-{slug}`, so the 1000th spec becomes `1000-slug`, which `parse_feature_dir` rejects (byte 3 is not `-`), making the directory invisible to every corpus reader. Pre-existing; the predicate's three-digit rule and the formatter's minimum-width pad have always disagreed. — runtime/src/primitives/create_feature.rs:117 and runtime/src/primitives/mod.rs (parse_sequential) (captured during 051-branch-scoped-spec-numbering)
