...
- [ ] Stale old-project-name env var survives the 049 rename: `runtime/src/primitives/fetch_archive.rs:278` defines `FETCH_ALLOW_INSECURE_HOSTS_ENV = "GVRN_FETCH_ALLOW_INSECURE_HOSTS"` (read at :285, reached from the SSRF guard at :225), and `runtime/tests/parity.rs:134,690` set it. 049's AC1 ("no live artifact under runtime/src/ references the old project name, except where recording history") is marked met but this is the live name an operator must set today, not a historical reference — and no audit family checks for it. Surfaced 2026-08-16 by /ductus:review on 013. Decide: rename to DUCTUS_* (a breaking change for anyone who set it), or accept-and-record it as a deliberate exception on 049.
