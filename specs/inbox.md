...
- [ ] other: cross-spec impact owed to `050-constitution` — 055 makes an adopter's governance multi-source, and 050 owns "which rules adopters should receive", so under §cross-spec-impact that belongs in 050 as an acceptance criterion or scenario with a back-link to 055. 050 is `done`, so it takes the `done → in-progress` back-edge via `/ductus:amend`. Recorded here because 055's §State at hand-off — its only previous home — was retired when that spec closed; the operator decided it does not block 055. — `specs/050-constitution/spec.md` (captured during /ductus:analyze)
