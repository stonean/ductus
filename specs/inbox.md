...
- [ ] bug: the completing `in-progress → done` status flip stales the analyze record by construction. `set-status` rewrites `spec.md`, which is an analyze subject, so the moment a spec reaches `done` its digest no longer matches — verified on 047 immediately after its own transition. Nothing is currently broken: the gate only runs at the transition, and `check-artifacts`' `analyze-state-drift` family deliberately checks presence and blocking rather than freshness. The reachable consequence is a confusing one — re-running `/{project}:implement` against an already-`done` spec reports `analysis is stale — spec.md` and tells the operator to re-run `/{project}:analyze` on work that is finished. Do **not** fix by excluding `status:` from the digest (families condition on status, so the digest must see it) or by re-recording after the flip (that asserts a run nothing on disk substantiates — the reason backfilling was rejected in 047 AC11). The likely shape is for the gate to recognise that a spec already at `done` has no transition to gate. Routes to 022 (gate behavior) or 047 (record semantics); `/groom` decides.
