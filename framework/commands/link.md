...
- Reference: §text-first-artifacts, §runtime-boundary, and [030 `data-model.md`](https://github.com/stonean/ductus/blob/main/specs/030-cross-service-references/data-model.md) — the canonical source for the `[services]` schema (constitution loaded by `/{project}:target`; do not re-read).
...
- Read `.ductus/config.toml`. If `[services]` is absent or empty, report that no services are registered and show the register form (`/{project}:link <alias> <repo> <path>`).
