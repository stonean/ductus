...
Retroactive declaration. `/{project}:specify --supersedes <feature>` captures the relation at creation, which is the moment it is cheapest; this command captures it over **two specs that already exist**, producing the same key and the same annotation. A corpus adopted before this feature existed carries conflicts nobody declared, and a flag on spec creation reaches only specs not yet written — so this is the path an adopting project uses to clean one up, per [052 — Spec supersession and consolidation](../../specs/052-spec-supersession-and-consolidation/spec.md).

Supersession **keeps** the earlier spec. It stays on disk, annotated, as the record of what shipped. The opposite operation — merging a spec into another and removing its directory — is `/{project}:consolidate`, and the two must not be confused: a spec that shipped and was later countered is the historical record of what shipped, while a spec that overlapped a sibling from the start was never a separate concern.
...
- Reference: §spec-requirements (the annotation, at all three granularities), §spec-lifecycle (mechanical edits and back-edges), §text-first-artifacts (the frontmatter schema, and which pointer fields are hand-authored), §spec-phase (spec-root resolution) (constitution loaded by `/{project}:target` — do not re-read).
...
**An interrupted declaration is completed by re-running it.** The runtime provides no transaction spanning two specs — the condition `/{project}:fold` documents for the same reason — so the recovery for an interruption between the two writes is not a rollback but a second run, and each write is built so a re-run is a no-op where the first attempt landed:
...
5. Invoke `read-supersession-pair` to load what reconciliation classifies: the two declared specs and the superseded spec's scenarios. The bound is the primitive's, not this step's — there is no argument by which a plan, a data model, a tasks file, a source path, or a third spec could be requested, so the read cannot widen by anyone forgetting. Carry its `unreadable` list forward: a file that would not parse contributes no claim, is never escalated into a conflict, and must not vanish either.

6. <!-- llm:classifyClaims --> Classify each of the superseded spec's claims against what the superseding spec declares it removes. Four outcomes, closed: **superseded** (the superseding spec removes it), **still standing** (untouched), **conflicting** (contradicted without being removed), **unclassified** (not decidable from the declared pair). Return every claim examined, *including* the still-standing ones — omitting them makes examined-and-untouched indistinguishable from never-looked-at. Each verdict but `still-standing` carries one sentence of reasoning quoting what drove it, because an operator settling a conflict needs the evidence and a bare verdict is not reviewable. **This is not the rejected check**: that one hunted unscoped pairs for supersessions nobody declared and fired on 215 of 455 with every sample a false positive; a declared edge collapses the search to two specs and changes what a false positive costs — a candidate offered to an operator who has already said these two conflict.

7. Invoke `write-supersession-annotation` once per **superseded** claim, passing its `AC{n}` label as the criterion. The criterion is annotated, never edited: it stays ticked with its text unchanged, because it *was* delivered and the removal belongs to the later spec — editing it would falsify a delivery record. The citation is by name, since a criterion is a plain list item with no blockquote exemption and a link there would be harvested into this spec's own `dependencies:`. A claim classified anything else is left alone.

8. Surface the conflicts, and resolve none of them (host responsibility). Name each conflicting claim with its rationale and both sides. Reconciliation reports; the operator decides, and never by this command picking a side. A **body-prose** claim may be edited where it genuinely reads better as current state, but only behind `gate-confirm` with a prompt naming the `done → in-progress` back-edge the edit causes *before* it happens — an acceptance criterion is never offered for editing at all.

<!-- audit:ignore-promotion -->
9. Report the declaration and the reconciliation (host responsibility): the two specs, which writes landed and which were already applied, the superseded spec's status — unchanged, because both writes are mechanical edits — and the classification. **Say which of three states this run reached**, because they must not read alike: *examined, with conflicts to settle*; *examined, nothing to reconcile* (the superseded spec carries no classifiable claims — a brownfield sketch legitimately does); or *could not fully examine*, naming the files rather than folding them into a total. A run is complete only when nothing was unreadable and the pair had something to classify. Recommend committing: the reciprocity check reads the declared edge against the annotated body, so it has something to check only once both writes are on disk together.
...
This section is the **single canonical statement** of what declaring a supersession means. `/{project}:specify --supersedes` performs the same declaration at creation time and points here rather than restating it; a second copy is how the two drift apart.
...
**Declaring reconciles.** The two writes above record the *relation*; reconciliation walks the superseded spec's individual claims and classifies each — superseded, still standing, conflicting, or unclassified. It runs **at declaration**, while the superseding spec's claims are being authored and the intent is held, for the same reason the declaration itself does: the information is cheap now and effectively unrecoverable later. Deferring it to the superseding spec's completion gate would re-create the recovery problem the declaration exists to avoid.

Its read is bounded to the declared pair plus the superseded spec's scenarios — no plan, no data model, no tasks file, no source tree, no third spec — and the bound lives in `read-supersession-pair` rather than in this prose, so it cannot widen by anyone forgetting. That bound is what separates reconciliation from the corpus-wide check this framework measured and rejected.

**Annotation is the default; editing is never automatic.** The two are not symmetric in cost: a meaningful body edit to a `done` spec takes the `done → in-progress` back-edge, while an annotation is a mechanical edit that leaves the status alone — and reopening is the wrong signal, since a superseded spec is not unfinished but finished and later countered. An acceptance criterion is **never** edited; body prose may be, on a confirmation that names the reopen first.

**A conflict is surfaced, never resolved.** Reconciliation reports what contradicts; it does not pick a side, and no primitive consumes the conflict list.

**Reconciliation says what it could not do.** A superseded spec with no classifiable claims is *examined and empty*, not *examined and clean*; a spec or scenario that will not parse is named and excluded from the counts rather than counted as reconciled. From the pair alone reconciliation can determine what the superseding spec **declares it removes** — it cannot determine that a claim was never delivered at all, which needs a read of the tree the bound forbids, so such a claim is reported unclassified and that call stays with the criterion-verification pass.

**The edge is descriptive.** It names a completed fact, so it gates no transition and appears on no backlog surface — unlike `folds-into:`, which blocks precisely because it names work not yet done. What verifies it is the `supersession-reciprocity` check surfaced through `/{project}:analyze`, which asks only whether the named spec's body names the superseding spec back. Its coverage is bounded to **declared** edges: a corpus of hand-written annotations carrying no key is invisible to it, and it reports that bound rather than presenting such a corpus as clean.
