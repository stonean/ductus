...
1. **`unregistered`** — the entry's `service` is null, or names an alias absent from `.ductus/config.toml` `[services]`. Status not attempted; a plain navigational link, not an error. (Surface it with a pointer to `/{project}:link` to register the service.)
