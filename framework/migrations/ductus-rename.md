...
**Summary:** Converge a project named for the retired `govern` / `gvrn` identifiers onto `ductus` — the per-project directory, the MCP server registration, the agent permission entries, the installed bootstrap entry point, the pre-commit hook and its invoker, and the two managed-block markers.
...
8. **Rewrite the MCP registration.** In each agent's MCP target, rename the server key `gvrn` to `ductus` and rewrite its `command` from `gvrn` to `ductus`, preserving `args` and every other server entry byte-for-byte:
...
9. **Rewrite the permission entries.** In each agent's settings file, rewrite the runtime tool-permission entries in that agent's own grammar, leaving every other entry untouched:
...
10. **Rewrite the managed-block markers.** Both markers delimit regions `/ductus` rewrites on every run, and `merge-managed-block` keyed to a new marker would **append a second block** rather than update the old one, leaving the adopter with two:
...
11. **Summary line.** When anything changed, report `renamed govern → ductus: {comma-separated list of what changed}` in the post-scaffolding output. Omit the line entirely when nothing changed.

## Notes

- The migration is one-way. There is no reverse path.
- Step 10 is the step whose omission is silent: nothing errors when a marker is missed, the next `/ductus` run simply appends a second managed block and the adopter's `.gitignore` grows a duplicate region on every run afterward.
