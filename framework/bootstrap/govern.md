...
1. For **each stale agent**, write the freshly fetched upstream to the agent's installed `ductus` file (overwrite) so the next session loads the up-to-date instructions. For `claude-style`, copy `{tempdir}/ductus.md.upstream` verbatim to `{config_dir}/commands/ductus.md`; for `opencode`, copy it verbatim to `{config_dir}/command/ductus.md`. For `antigravity`, write `{config_dir}/skills/ductus/SKILL.md` as the transformed skill — `name: ductus` frontmatter followed by the upstream body — **not** the raw `ductus.md` (a raw copy is not a loadable skill). In both cases do not substitute placeholders in the body — `{project}` and `{cli-config-dir}` stay literal, per the `ductus` self-install rule.
