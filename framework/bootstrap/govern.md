...
**Only `ductus`-namespaced tools count, and that holds for the whole run.** A pre-rename adopter can arrive with a retired server key still in their MCP config *and* a retired binary still resolvable — the rename migration declines to touch an installed binary, and the retired crate stays published rather than yanked — so the host launches that server at session start and its tools sit in the inventory for the entire session. Treat them as **absent**: for the remainder of this run a tool whose namespace is not `ductus` is not called, not preferred over the pointer CLI, and never read as evidence that a runtime is available. This is the same scoping that makes the detection above correct, carried past detection into execution — a host reaching for a live tool over a CLI call is otherwise following the general preference for the deterministic path, not disregarding an instruction. The reason is not tidiness: a retired-namespace server is a *different runtime at a different version*, and its primitives resolve paths against the directory layout of the release that shipped them. This run migrates that layout, so those resolvers are wrong by construction — the migration is what makes them wrong — and their writes land in the pre-migration location with a success result. The rule binds until the run ends rather than until the rename removes the key, because an MCP server is spawned at session start and is not torn down when its registration is deleted.
