...
pub fn run(args: &CheckArtifactsArgs, repo: &Path) -> Result<CheckArtifactsResult> {
    ...
    check_supersession_reciprocity(
        &mut findings,
        &mut skipped,
        &args.feature,
        &spec,
...
/// (i) Supersession reciprocity — a declared `supersedes:` edge whose named
/// spec does not name this one back (spec 052). **Advisory.**
///
/// **This is not the check that was rejected.** Detecting supersession was
/// measured across unscoped criterion pairs — 455 tested, 215 would fire,
/// every sample a false positive — and killed on that evidence. This family
/// asks a different question. The edge is *declared*, so it never infers a
/// relation: it reads `supersedes: [X]` and checks whether X's body mentions
/// this spec. There is no inference, so there is no false-positive class.
///
/// **Its coverage is bounded to declared edges.** A corpus full of
/// hand-written sunset annotations carries no `supersedes:` keys, so this
/// family sees none of it, and a silent pass over such a corpus must not be
/// read as assurance that no supersession is unrecorded — an assurance
/// nothing here can give, since deriving those edges is the rejected
/// inference. The bound is stated in this contract and rendered by
/// `/{project}:analyze`; it is deliberately **not** a per-spec `skipped`
/// entry, because that array names targets that could not be reached and a
/// spec declaring no edges has no target at all. Overloading it would mark
/// every spec in a corpus while telling an operator nothing actionable.
///
/// Reciprocity is satisfied by any blockquote line in the target naming this
/// spec — link form or bare name. The blockquote scoping is deliberate: an
/// ordinary prose link to this spec is a dependency-inducing reference and
/// says nothing about whether the annotation was written.
fn check_supersession_reciprocity(
    findings: &mut Vec<ArtifactFinding>,
    skipped: &mut Vec<SkippedTarget>,
    feature: &str,
    spec: &ReadSpecResult,
    ...
    let declared = &spec.frontmatter.supersedes;

    // No declared edges means no subject, not an unexamined target. The
    // coverage bound is real and is stated in this family's contract and in
    // `/{project}:analyze`'s rendering — but it does not belong in `skipped`,
    // whose closed reason set names targets that could not be reached.
    // Recording one per spec would put an informational line on every spec in
    // a corpus while telling an operator nothing they can act on.
    if declared.is_empty() {
        return;
    }

    for target in declared {
        let target_path = repo.join(root).join(target).join("spec.md");
        let target_rel = format!("{root}/{target}/spec.md");

        if !target_path.is_file() {
            record_skip(skipped, "target-missing", &target_rel);
            continue;
        }
        let Ok(content) = std::fs::read_to_string(&target_path) else {
            record_skip(skipped, "artifact-unreadable", &target_rel);
            continue;
        };
        ...
        if !names_back {
            findings.push(ArtifactFinding {
                family: "supersession-reciprocity".into(),
                severity: "advisory".into(),
                message: format!(
                    "{feature} declares it supersedes {target}, but {target}'s body carries no \
                     annotation naming {feature} — a reader opening it has no way to know it \
                     was countered"
                ),
                path: target_rel,
            });
...
mod tests {
    ...
    fn spec_superseding(targets: &str) -> String {
        format!("---\nstatus: done\ndependencies: []\nsupersedes: [{targets}]\n---\n\n# Demo\n")
    }

    #[test]
    fn a_declared_edge_the_target_names_back_is_clean() {
        let tmp = tempdir().unwrap();
        write(
            tmp.path(),
            "specs/042-demo/spec.md",
        ...
        write(
            ...
            "---\nstatus: done\ndependencies: []\n---\n\n# Workflows\n\n             > **Sunset ([042-demo](../042-demo/spec.md)):** removed.\n",
        );
        let result = run(&args(), tmp.path()).unwrap();
        assert!(
            !result
    ...
    #[test]
    fn a_declared_edge_the_target_ignores_is_advisory() {
        let tmp = tempdir().unwrap();
        write(
            tmp.path(),
            "specs/042-demo/spec.md",
        ...
        let f = result
            .findings
            .iter()
            .find(|f| f.family == "supersession-reciprocity")
    ...
    #[test]
    fn an_ordinary_prose_link_does_not_satisfy_reciprocity() {
        // A prose link to the superseding spec is a dependency-inducing
        // reference; it says nothing about whether the annotation was
        // written, and counting it would let a spec look annotated because
        // something else happened to cite its successor.
        let tmp = tempdir().unwrap();
        write(
            tmp.path(),
            "specs/042-demo/spec.md",
        ...
        write(
            tmp.path(),
            "specs/005-workflows/spec.md",
            "---\nstatus: done\ndependencies: []\n---\n\n# Workflows\n\n             See [042-demo](../042-demo/spec.md) for related work.\n",
        );
        let result = run(&args(), tmp.path()).unwrap();
        assert!(
            result
                ...
                .iter()
                .any(|f| f.family == "supersession-reciprocity"),
            "a prose link must not read as an annotation"
        );
    }

    #[test]
    fn a_spec_declaring_no_edges_produces_neither_finding_nor_skip() {
        // The family has no subject here, which is not the same as an
        // unexamined target — so it stays out of `skipped`, whose reason set
        // names things that could not be reached. The coverage bound is a
        // contract statement, not a per-spec entry; expressing it there would
        // mark every spec in a corpus.
        let tmp = tempdir().unwrap();
        write(tmp.path(), "specs/042-demo/spec.md", &spec("done", None));
        let result = run(&args(), tmp.path()).unwrap();
        assert!(
        ...
        assert!(
            !result
                .skipped
                .iter()
                .any(|s| s.family == "supersession-reciprocity"),
            "an empty subject is not an unexamined target: {:?}",
            result.skipped
        );
    }

    #[test]
    fn a_declared_edge_whose_target_is_absent_is_skipped_not_a_finding() {
        // Nothing can be proven about a spec that is not there. Reporting it
        // as unannotated would manufacture a defect out of an absence.
        let tmp = tempdir().unwrap();
        write(
            tmp.path(),
            "specs/042-demo/spec.md",
            &spec_superseding("404-gone"),
        );
        let result = run(&args(), tmp.path()).unwrap();
        assert!(
            !result
        ...
        assert!(result.skipped.iter().any(|s| s.reason == "target-missing"));
    }
}
