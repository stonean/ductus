...
4. **When the spec is still `planned`**, invoke `set-status` to flip its status from planned to in-progress. Invoking `/ductus:implement` is itself the user's approval to begin work, so this transition is not separately gated — no confirmation is prompted, with or without `--auto`. The primitive guards against a stale "from" value so concurrent edits surface as an operational error rather than a silent overwrite. **Skip this step when the spec is already `in-progress`** — per-task runs and the completion gate re-invoke `/ductus:implement` on an in-progress spec (see steps 7–8 and the stuck-detection details), and calling `set-status` with `from: planned` on an in-progress spec would halt on the stale-`from` guard. Read the current status (from step 1's context, which already loaded the task list and target) and only transition on the planned → in-progress edge.
...
14. Invoke `check-review-gate` against the feature to evaluate the pre-done review gate: the feature directory's markdown lint (through the lint-markdown machinery, replacing the raw `npx markdownlint-cli2` invocation), then unresolved scenario open questions, then the spec frontmatter `review:` block, then whether the recorded review is still current. A blocked result names the first failing check (markdown lint violations; unresolved scenario questions; not reviewed; blocking MUST violations; review stale) and carries the canonical blocked message — plus the resolve-or-waive guidance on the MUST-violations branch and the re-run guidance on the stale branch; halt with that message and do not propose the transition. On `passed: true`, proceed — but surface any `guidance` the result carries even then: a passing gate names the durable contracts it could **not** examine (see the completion gate's step 5), and that notice is the operator's to weigh before approving the transition, not the walker's to drop. The full check order and message texts are documented in the completion gate's step 5 below, which is also the markdown-only path.

<!-- audit:ignore-promotion -->
15. If all checks pass, present a summary — including any issues captured to `specs/inbox.md` during this feature's implementation (per §brownfield-inbox Automatic issue capture), each listed with a pointer to run `/ductus:groom` to route them — and ask the user to approve the transition to done. Do not update the status until the user confirms; on denial, the walker exits cleanly without writing.
...
5. **Route an incidental issue by scope.** When implementing this task surfaces an issue — a security weakness, a memory or resource leak, a violated convention, a latent bug in adjacent code — the scope it falls in decides where it goes (§brownfield-inbox Automatic issue capture). An issue *inside this task's* scope is fixed as part of the task, not logged. An issue inside the **targeted spec's** scope but outside this task becomes a new unchecked task on that spec's `tasks.md` (deterministic path: the `append-task` primitive) — never an inbox item, since the inbox would route it back to the spec that is already open. Only an issue outside the spec entirely is captured to `specs/inbox.md` automatically, without prompting (deterministic path: the `append-inbox` primitive, whose dedup guard keeps a re-run from double-logging the same finding); without the runtime, append the bullet with the host's file tools. In every case, record and keep working — do not derail to fix a finding outside this task. A capture follows the inbox auto-capture form (see `specs/inbox.md`); a finding that is a requirement gap rather than a defect routes through `/ductus:amend` per §bug-handling, with the task as the work item that implements it.
...
5. **Pre-done review gate** (primitive: `check-review-gate`, which evaluates all four checks below in this order and returns the verdict with the canonical blocked message). First confirm all `.md` files in the feature directory pass `npx markdownlint-cli2`; on failure, report the specific violations and do not propose the transition. Then confirm no scenario under the feature carries unresolved open questions: a scenario is an organizational split of the spec, so its questions are the spec's questions for completeness and the spec is not `done` while any remain (§spec-lifecycle; spec 046). On failure, halt with `blocked: {N} unresolved open question(s) in scenario(s) {names} — resolve them before completing`, followed by guidance to run `/ductus:target {feature}/<scenario>` then `/ductus:clarify` on each — or, when a question is deferred rather than undecided ("not now; revisit when X lands"), to record it in that scenario's `## Resolved Questions` with its trigger, since a deferral is a resolution with a condition and only `## Open Questions` entries count. This check is ordered ahead of the `review:` block because an unresolved design question is the more upstream defect: reviewing a design that is about to change wastes the review. Then read the target spec's frontmatter `review:` block before asking for the done transition. If `review.last-run` is missing, null, or the review block is absent, halt with: `blocked: spec has not been reviewed — run /ductus:review before completing`. If `review.blocking: true`, halt with: `blocked: spec has {must-violations} MUST violation(s) — see specs/NNN-feature/review.md` followed by guidance to either resolve the violations and re-run `/ductus:review`, or run `/ductus:review --waive <rule-id> --reason "..."` for each waivable finding.

   Finally, confirm the recorded review still describes the current code. Diff the spec's **durable contracts** — `scenarios/*.md` and `data-model.md`, the artifacts a review actually reads — between `review.reviewed-against` and `HEAD`; `tasks.md`, `plan.md`, `review.md`, and `spec.md` are deliberately out of scope, and a contract that changed only by a repo-wide rename says what it said before and is not stale. When any remain, halt with: `blocked: review is stale — {N} durable contract(s) changed since reviewed-against {sha}: {paths}` followed by guidance to re-run `/ductus:review` so the recorded verdict describes the current code. This check is ordered last because it is the weakest claim — the others say a review is missing or failing, this one says a passing review is out of date. Without it, a review that predates the code it nominally covers satisfies every other check. Otherwise, proceed.

   **Say what could not be examined.** The diff above compares committed trees, so a durable contract that exists only in the working tree is invisible to it — and at the moment this gate runs that is the *normal* state, not an edge case: a scenario written during the session is uncommitted, `reviewed-against` is HEAD, the diff is empty, and the gate reports clean while the staleness it exists to prevent becomes real on the next commit. So before proceeding, check whether any durable contract under the feature has uncommitted changes — modified, staged, or untracked — and when any does, report `guidance` naming those paths and stating that staleness could not be determined against them. This does **not** block: `passed` is unchanged and committing before reviewing is a workflow choice, not an error. What it prevents is a clean verdict that is silent about its own blind spot (`QUAL-CLAIM-001` applied to the gate itself — the same reasoning `stale_review_block` already records against an unresolvable `reviewed-against`, where a check that silently could not run failed open with no signal). A genuine stale block above wins over this notice; the guidance is not a softer substitute for a check that actually fired. When every durable contract is committed, emit nothing — the common case stays quiet, so its silence means "examined and current".
6. If all checks pass, present a summary — including any issues captured to `specs/inbox.md` during this feature's implementation (per §brownfield-inbox Automatic issue capture), each listed with a pointer to run `/ductus:groom` to route them — and ask the user to approve the transition to done. Do not update the status until the user confirms.
