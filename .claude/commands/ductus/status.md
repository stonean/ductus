...
<!-- audit:ignore-promotion -->
1. List any non-done specs (excluding the current target, if any) and prompt the user to run `/ductus:target` to select one.

## Status → next action

| Status | Next Action |
| --- | --- |
| draft | /ductus:clarify |
| clarified | /ductus:plan |
| planned | /ductus:implement |
| in-progress | /ductus:implement |
| done | done (spec is complete) |

When a scenario is targeted and the scenario itself has one or more open questions, the next action is `/ductus:clarify` (scenario-targeted, resolves scenario-level open questions regardless of parent spec status).
...
1. **`unregistered`** — the entry's `service` is null, or names an alias absent from `.ductus/config.toml` `[services]`. Status not attempted; a plain navigational link, not an error. (Surface it with a pointer to `/ductus:link` to register the service.)
2. Otherwise read the matched `[services.<alias>]` entry's local `path` (relative to the repo root, or absolute):
   ...
   - Otherwise resolve the target spec at `{path}/{linked-specs-root}/{spec}/spec.md`, where `{linked-specs-root}` is the **linked service's own** `[paths] specs-root` (default `specs`) read from the checkout's `.ductus/config.toml` — each service may configure its own spec root, and `resolve-references` honors the linked repo's setting, so the markdown-only path must read it too rather than assuming `specs/`:
     - **`broken`** — the target file does not exist (renamed, moved, deleted, or mistyped upstream). A provable defect, surfaced by `/ductus:analyze`.
