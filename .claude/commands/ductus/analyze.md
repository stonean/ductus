...
- Never modify an artifact under audit. Three writes are in scope and no others: the inbox capture of surviving findings (step 16), the spec's `analyze:` frontmatter block (step 18), and — only with `--fix` — the revert of a drifted `done` spec from `done` to `in-progress` via `set-status` on either trigger — review-state drift (see Review state drift below) or unresolved scenario open questions (see Scenario open questions below). Without `--fix`, no file is written.
