...
### Supersession reciprocity (advisory)

For each entry in the spec's `supersedes:` frontmatter list, read the named spec and confirm its body carries a blockquoted annotation naming this spec. Report an entry whose target does not: a reader opening that spec has no way to know it was countered.

Reciprocity is satisfied by any **blockquote** line naming this spec — link form or bare name. The blockquote scoping is deliberate: an ordinary prose link is a dependency-inducing reference and says nothing about whether the annotation was written, so counting one would let a spec look annotated because something else happened to cite its successor.

A named target that is absent or unreadable is **skipped**, never reported as unannotated — nothing can be proven about a spec that is not there, and flagging it would manufacture a defect out of an absence.
