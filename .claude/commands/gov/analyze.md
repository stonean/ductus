...
- **No grandfather rule**, unlike Review state drift below. An absent `review:` block genuinely marks a spec as predating `/gov:review`; an unresolved scenario question is a present-tense defect whenever it arrived, and exempting it would preserve exactly the state this check exists to surface.
...
Suggested fix per finding, offering both exits: resolve each question via `/gov:target {feature}/<scenario>` then `/gov:clarify` (scenario-targeted), **or** — when the question is deferred rather than undecided — move it to the scenario's `## Resolved Questions` with the condition that will settle it.
...
Enforces `.govern/constitution.md` §drift-prevention's *Decision resolution* obligation where it can be checked deterministically: an artifact's own prose asserting an open state that its own sibling link's target contradicts. The grounding check above cannot catch this — it verifies a claim is *cited*, not that it is *true*, so a stale claim that correctly cites its source passes.
...
Suggested fix per finding: correct the criterion to name the current path, or — when the deletion means the criterion is no longer satisfied — uncheck it and route the work through `/gov:amend`.
